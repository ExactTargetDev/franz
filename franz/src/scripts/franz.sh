...
HEAP_OPTS="-Xmx1024m -Xms1024m -XX:NewSize=768m"
...
case "$1" in
  start)
    ...
    $DAEMON $daemon_args $daemon_start_args -- sh -c "echo "'$$'" > $pidfile; exec ${JAVA_HOME}/bin/java ${JAVA_OPTS} -jar ${APP_HOME}/${JAR_NAME} -f ${APP_HOME}/config/production.scala"
    tries=0
    while ! running; do
      tries=$((tries + 1))
      if [ $tries -ge 5 ]; then
